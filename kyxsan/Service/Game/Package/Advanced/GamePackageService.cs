...
internal sealed partial class GamePackageService : IGamePackageService
{
    ...
    public async ValueTask<bool> ExecuteOperationAsync(GamePackageOperationContext operationContext)
    {
        ...
        using (IServiceScope scope = serviceProvider.CreateScope())
        {
            ...
            GamePackageOperationDialog dialog = await contentDialogFactory
                .CreateInstanceAsync<GamePackageOperationDialog>(scope.ServiceProvider)
                .ConfigureAwait(false);
            IProgress<GamePackageOperationReport> progress = scope.ServiceProvider
                .GetRequiredService<IProgressFactory>()
                .CreateForMainThread<GamePackageOperationReport>(dialog.HandleProgressUpdate);

            ValueContentDialogTask dialogTask = contentDialogFactory.EnqueueAndShowAsync(dialog);
                ...
                using (TokenBucketRateLimiter? limiter = StreamCopyRateLimiter.Create(serviceProvider))
                {
                    ...
                    catch (OperationCanceledException)
                    {
                        if (operationCts is { IsCancellationRequested: true })
                        {
                            ...
                            await dialogTask.ShowTask.ConfigureAwait(false);
                            return false;
                        }
                    ...
                    catch (Exception ex)
                    {
                        ...
                        progress.Report(new GamePackageOperationReport.Abort(ex.Message));
                        result = false;
                    }
                    finally
                    {
