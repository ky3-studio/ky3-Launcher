...
internal sealed partial class DescriptionTextBlock : ContentControl
{
    ...
    private void UpdateDescription(TextBlockType textBlock, string? text)
    {
        ...
        // Truncate excessively long texts to prevent UI thread blocking
        const int MaxTextLength = 2000;
        string displayText = text.Length > MaxTextLength
            ? string.Concat(text.AsSpan(0, MaxTextLength), "\u2026")
            : text;

        string content = SpecialNameHandling.Handle(displayText);
