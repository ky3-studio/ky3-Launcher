...
internal sealed partial class SpiralAbyssView : IEntityAccess<SpiralAbyssEntry?>, IPropertyValuesProvider
{
    ...
    {
        return [.. ranks.Where(r => r.AvatarId != 0U).Select(r => new RankAvatar(r.Value, context.GetAvatar(r.AvatarId)))];
    }
    ...
    {
        return ranks.Where(r => r.AvatarId != 0U).Select(r => new RankAvatar(r.Value, context.GetAvatar(r.AvatarId))).SingleOrDefault();
    }
}