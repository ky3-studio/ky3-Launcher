...
internal sealed partial class UserViewModel : ObservableObject
{
    ...
    internal void HandleUserOptionResult(UserOptionResultKind optionResultKind, string? uid)
    {
        switch (optionResultKind)
        {
            case UserOptionResultKind.Added:
                ...
                taskContext.InvokeOnMainThread(Users.MoveCurrentToLast);

                messenger.Send(InfoBarMessage.Success(SH.FormatViewModelUserAdded(uid)));
                AutoAddDailyNoteAsync().SafeForget();
                break;
            ...
            case UserOptionResultKind.CookieUpdated:
                ...
                {
                    // ResumeUserAsync replaced UserGameRoles with a new collection
                    // (CurrentItem = null). Users.Refresh() won't trigger
                    // OnCurrentUserChanged because the same user stays at the same
                    // position, so MoveCurrentTo short-circuits via ReferenceEquals.
                    // Force re-trigger by moving to -1 first, then back.
                    if (Users.CurrentItem is { } current)
                    {
                        Users.MoveCurrentToPosition(-1);
                        Users.MoveCurrentTo(current);
                    }
                });
    ...
    private async Task AutoAddDailyNoteAsync()
    {
        try
        {
            await taskContext.SwitchToMainThreadAsync();

            User? currentUser = Users?.CurrentItem;
            ...
            await taskContext.SwitchToBackgroundAsync();
            await userService.RefreshCookieTokenAsync(currentUser.Entity).ConfigureAwait(false);
            ...
            DailyNoteMetadataContext context = await metadataService.GetContextAsync<DailyNoteMetadataContext>().ConfigureAwait(false);
            IDailyNoteService dailyNoteService = serviceProvider.GetRequiredService<IDailyNoteService>();

            foreach (UserGameRole role in currentUser.UserGameRoles.Source)
            {
                UserAndUid userAndUid = new(currentUser.Entity, role);
                await dailyNoteService.AddDailyNoteAsync(context, userAndUid).ConfigureAwait(false);
            }
        }
        catch
        {
