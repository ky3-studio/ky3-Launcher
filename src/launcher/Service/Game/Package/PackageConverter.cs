...
internal sealed partial class PackageConverter : IPackageConverter
{
    ...
    public async ValueTask<bool> EnsureGameResourceAsync(PackageConverterContext context)
    {
        ...
        ValueTask<SophonDecodedBuild?> currentBuildTask = DecodeManifestsAsync(context, context.CurrentBranch, context.CurrentScheme);
        ValueTask<SophonDecodedBuild?> targetBuildTask = DecodeManifestsAsync(context, context.TargetBranch, context.TargetScheme);
        SophonDecodedBuild? currentBuild = await currentBuildTask.ConfigureAwait(false);
        SophonDecodedBuild? targetBuild = await targetBuildTask.ConfigureAwait(false);
    ...
    {
        await Parallel.ForEachAsync(operations, context.ParallelOptions, async (operation, _) =>
        {
