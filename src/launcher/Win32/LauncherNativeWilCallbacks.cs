...
internal static unsafe class LauncherNativeWilCallbacks
{
    public static void LauncherInitializeWilCallbacks()
    {
        Marshal.ThrowExceptionForHR(LauncherInitializeWilCallbacks(&WilLoggingImpl, &WilMessageImpl));
    }

    [SuppressMessage("", "SYSLIB1054")]
    [DllImport(LauncherNativeMethods.DllName, CallingConvention = CallingConvention.Winapi, ExactSpelling = true, EntryPoint = "HutaoInitializeWilCallbacks")]
    private static extern HRESULT LauncherInitializeWilCallbacks(delegate* unmanaged[Stdcall]<FailureInfo*, void> loggingCallback, delegate* unmanaged[Stdcall]<FailureInfo*, PWSTR, ulong, void> messageCallback);

    [UnmanagedCallersOnly(CallConvs = [typeof(CallConvStdcall)])]
    private static void WilLoggingImpl(FailureInfo* failure)
    {
        LauncherNativeException exception = new(*failure);
...
#pragma warning restore CS0649
    ...
    internal sealed class LauncherNativeException : Exception
    {
        public LauncherNativeException(FailureInfo info)
